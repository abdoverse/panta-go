...
class HelperJobCard extends StatelessWidget {
            ...
            Stack(
              children: [
                Container(
                  ...
                  child: HelperRequestImage(imageUrl: job.imageUrl),
                ),
                Positioned(
                  bottom: 0,
                  left: 0,
              ...
              children: [
                ...
                if (!isAcceptable && !isCompleted)
                  Padding(
                    ...
                    child: TimeRemainingDisplay(deadline: job.scheduledTo),
                  ),
                if (isCompleted)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                        ...
                        children: [
                          ...
                          Expanded(
                            child: FilledButton.icon(
                              onPressed: () async {
                                final scanResult =
                                    await ReceiptScannerDialog.show(
                                  context,
                                  requestId: job.id,
                                ...
                                if (!context.mounted || scanResult == null) {
                                  return;
                                }
                                ...
                                final completed = await context
                                    ...
                                    .completeRequest(
                                      ...
                                      splitPercentage:
                                          scanResult.splitPercentage,
                                      dropoffPhotoUrl:
                                          scanResult.dropoffPhotoUrl,
                                    );
                                if (!context.mounted) return;
                                if (!completed) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content:
                                          Text(l10n.couldNotCompletePickup),
                                    ),
                                  );
                                  return;
                                }
                                ...
                                if (!context.mounted) return;
                                final helperShare = (scanResult.amount *
                                        (100 - scanResult.splitPercentage)) /
                                    100;
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text(
  ...
  Future<void> _showCompletionCelebration(
    ...
    final dialogFuture = showDialog<void>(
      ...
      builder: (dialogContext) => CompletionCelebrationDialog(
...
class HelperRequestImage extends StatelessWidget {
...
class TimeRemainingDisplay extends StatelessWidget {
  ...
  const TimeRemainingDisplay({super.key, required this.deadline});
