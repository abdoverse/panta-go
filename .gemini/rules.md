...
- **Dev Loop:** Whenever you need to apply code changes, ALWAYS run `flutter build web` inside the `mobile` directory. When the build finishes, auto-commit, and play the notification sound. The user will refresh their own static browser window on port 3000.
- **Visual Debug Banner:** Whenever you complete a task or fix, ALWAYS update the text in the red debug banner inside `mobile/lib/features/dashboard/user_home_page.dart` to concisely describe the latest changes made. This proves to the user that the app has successfully recompiled and gives them immediate visual confirmation of the fix.
