...
- **Flutter Web Debugging Port:** If you ever need to attach a debugger using `flutter run -d chrome`, ALWAYS use `--web-port=3001`. Port 3000 is managed by the user's automated `watch_commits.sh` script, which will brutally SIGKILL anything on port 3000 whenever a git commit occurs.
- **No Flutter Web Hot Reloads:** NEVER trigger a hot reload (`r`) when running `flutter run -d chrome`. Just read the logs while the user interacts, or do a full fresh restart if necessary.
