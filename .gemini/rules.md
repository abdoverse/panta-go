...
- **Dev Loop:** Whenever you need to apply code changes, ALWAYS run `flutter build web --debug --no-tree-shake-icons` inside the `mobile` directory. When the build finishes, auto-commit, and play the notification sound. The user will refresh their own static browser window on port 3000.
