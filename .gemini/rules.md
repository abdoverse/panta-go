...
- **Dev Loop:** I maintain a persistent `flutter run -d web-server --web-port=3000` task running in the background. Whenever you need to apply code changes, NEVER run `flutter build web`. Instead, use the `manage_task` tool with action `send_input` to send the letter "r" to the persistent task to trigger an instant hot-reload! After triggering the reload, auto-commit, and play the notification sound. The user will immediately see changes on port 3000 without waiting 4 minutes.
